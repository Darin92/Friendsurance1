...
public class ChangeUserPassword implements LoginVariables{
    ...
    public static void ChangePassword() {
       ...
       //Verify List Expenses text is present that is meant that user is already logged in
        Assert.assertTrue(!driver.findElements(By.xpath(".//h1[contains(text(), \"List Expenses\")]")).isEmpty(),
                "No List Expenses header on the page");

        //ChangePassword
        regNewUserPage.GoToEditAccoutPage();
        String passwordName3 = "Password0";
        String PasswordName4 = "Password0";
        regNewUserPage.ChangePassword(PasswordName,passwordName3, PasswordName4);

        //logout
        driver.findElement(By.xpath(".//*[@id='logout']")).click();

        //Login user with new password
        regNewUserPage.LoginAsCreatedUser(LoginName, passwordName3);
        //Verify List Expenses text is present that is meant that user is already logged in
        Assert.assertTrue(!driver.findElements(By.xpath(".//h1[contains(text(), \"List Expenses\")]")).isEmpty(),
                "No List Expenses header on the page");
    }
